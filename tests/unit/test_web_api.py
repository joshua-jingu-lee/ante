...
class TestRFC7807ErrorResponse:
    ...
    def test_openapi_auto_422_normalized_to_problem_json_error_response(self):
        """FastAPI 자동 생성 422도 ``application/problem+json`` + ``ErrorResponse``
        ``$ref``로 정규화된다 (#1681, invariant).

        글로벌 ``RequestValidationError`` 핸들러는 runtime
        ``media_type=PROBLEM_JSON`` + RFC 7807 ``ErrorResponse`` envelope로
        응답하지만, FastAPI 기본 OpenAPI는 자동 422를 ``application/json`` +
        ``HTTPValidationError`` ``$ref``로 노출해 codegen/문서 drift가
        발생했다(#1681 — 31개 endpoint). ``_install_openapi_customizer``의
        ``_normalize_autogenerated_validation_422`` 분기가 단일
        mechanism-agnostic 규칙으로 전 endpoint를 정렬한다.

        전수 sweep: 모든 path/method ``responses["422"].content`` 키는
        정확히 ``{"application/problem+json"}`` 이고 ``$ref`` 는
        ``#/components/schemas/ErrorResponse`` 이다. ``application/json`` /
        ``HTTPValidationError`` 를 갖는 422 잔존은 0건이어야 한다.

        ``HTTPValidationError`` schema 정의 자체는 보존된다 (본 정규화는 422
        ...
        for path, methods in schema.get("paths", {}).items():
            ...
            for method, spec in methods.items():
                ...
                if not isinstance(entry, dict):
                    continue
                total_422 += 1
                if (path, method) in required_samples:
                    seen_samples.add((path, method))
                content = entry.get("content")
                if not isinstance(content, dict):
                    violations.append(f"{path} {method.upper()} 422: content map 없음")
                    continue
                media_keys = set(content.keys())
                if media_keys != {"application/problem+json"}:
                    violations.append(
                        f"{path} {method.upper()} 422: {sorted(media_keys)} "
                        f"expected only application/problem+json"
                    )
                    continue
                schema_obj = content["application/problem+json"].get("schema")
                ref = schema_obj.get("$ref") if isinstance(schema_obj, dict) else None
                if ref != "#/components/schemas/ErrorResponse":
                    violations.append(
                        f"{path} {method.upper()} 422: $ref={ref!r} "
                        f"expected #/components/schemas/ErrorResponse"
                    )

        assert violations == [], (
            "OpenAPI 422 응답이 problem+json + ErrorResponse로 정규화되지 "
            "않음 (#1681):\n" + "\n".join(f"  - {v}" for v in violations)
        )
        assert total_422 > 0, "검사 대상 422 응답이 없음 — sweep 무력화 의심"
        assert seen_samples == required_samples, (
            "이슈 본문 sample 422 endpoint 일부가 OpenAPI에 없음: "
            f"missing={sorted(required_samples - seen_samples)}"
        )

        # HTTPValidationError schema 정의 자체는 보존 (Non-Goal).
        schemas = schema.get("components", {}).get("schemas", {})
        assert "HTTPValidationError" in schemas, (
            "HTTPValidationError schema 정의가 제거됨 — 본 이슈 Non-Goal "
            "위반 (#1681: content-type/$ref만 재작성, 정의는 보존)"
        )
