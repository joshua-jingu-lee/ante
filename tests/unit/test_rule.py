...
class TestRuleEngineEventBus:
    ...
    @pytest.mark.parametrize(
        ("bad_price", "bad_side"),
        [(-1000, "buy"), (-1000.0, "buy"), (-1000.5, "sell")],
    )
    async def test_rule_engine_rejects_negative_price(
        ...
        fail-closed로 거부한다. 정수/실수 음수, buy/sell 모두 잠근다.
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for negative price")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError("_query_treasury_data must not run for negative price")

        async def _spy_unrealized(bot_id, current_price, order_symbol):
            unrealized_calls.append((bot_id, current_price, order_symbol))
            raise AssertionError(
                "_calculate_bot_unrealized_pnl must not run for negative price"
            )
        ...
        order = OrderRequestEvent(
            ...
            price=bad_price,
            exchange="KRX",
            reason="A7 oracle negative-price regression",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        assert "Negative price" in ev.reason
        assert ev.account_id == "domestic"
        assert ev.bot_id == "bot1"
        assert ev.strategy_id == "s1"
        assert ev.symbol == "005930"
        ...
        assert ev.quantity == 1.0
        assert ev.order_id == str(order.event_id)
        # rejection payload의 price는 finite 음수 그대로 보존된다.
        assert ev.price == float(bad_price)

    async def test_rule_engine_rejects_negative_price_stop_limit(
        self, engine, eventbus, monkeypatch
    ):
        """order_type='stop_limit'에서도 음수 price는 본 게이트로 거부된다.

        본 게이트는 order_type 무관하게 price<0를 잠그므로, stop_limit 주문에
        음수 price가 실려도 동일하게 fail-closed로 거부되어야 한다 (#1325 같은
        cross-order_type 회귀를 함께 잠근다).
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for negative stop_limit price")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError(
                "_query_treasury_data must not run for negative stop_limit price"
            )
        ...
        order = OrderRequestEvent(
            ...
            stop_price=600.0,
            exchange="KRX",
            reason="negative price stop_limit regression (#1325)",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        assert "Negative price" in ev.reason
        assert ev.price == -500.0
        assert ev.order_type == "stop_limit"
        assert ev.order_id == str(order.event_id)

    @pytest.mark.parametrize("good_price", [0.001, 1, 1000.0])
    async def test_rule_engine_accepts_positive_price_after_negative_gate(
        self, engine, eventbus, good_price
    ):
        """양수 price는 본 게이트를 통과해 OrderValidatedEvent 발행.

        본 PR(#1316)은 음수 price만 거부한다. 양수 finite price는 계속 통과해야
        하며, 기존 정상 흐름이 깨지지 않는지 회귀 잠금. 정수/실수, 매우 작은
        양수까지 잠근다.
        """
        validated: list[OrderValidatedEvent] = []
        rejected: list[OrderRejectedEvent] = []
        ...
        order = OrderRequestEvent(
            ...
            price=good_price,
            exchange="KRX",
            reason="positive price should pass negative-price gate",
        )
        await eventbus.publish(order)
    ...
    async def test_rule_engine_negative_price_fires_before_negative_quantity_gate(
        ...
        """price-negative 게이트가 quantity-negative(#1304) 게이트보다 먼저 fire.

        cross-field invalid (quantity=-1, price=-1000) payload에서 본 게이트가
        먼저 fire하여 reason에 "Negative price"가 실린다. 게이트 배치 순서
        invariant 잠금: 향후 누군가 게이트 순서를 바꾸면 이 테스트가 fail해서
        시정 필요성을 알린다. ``_coerce_finite_quantity``는 finite 음수를
        보존하므로 ev.quantity는 -1.0 그대로 audit trail에 실린다.
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for negative price/quantity")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError(
                "_query_treasury_data must not run for negative price/quantity"
            )
        ...
        order = OrderRequestEvent(
            ...
            price=-1000,
            exchange="KRX",
            reason="cross-field negative price + negative quantity",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        # price 게이트가 먼저 fire — reason은 "Negative price" (NOT "Negative quantity")
        assert "Negative price" in ev.reason
        assert "Negative quantity" not in ev.reason
        # audit 보존: 음수 finite price/quantity가 그대로 payload에 실린다.
        assert ev.price == -1000.0
        assert ev.quantity == -1.0
    ...
    async def test_rule_engine_rejects_zero_stop_price_stop(
        self, engine, eventbus, monkeypatch, bad_stop_price, bad_side
    ):
        """stop_price==0 + order_type='stop'이면 룰 평가/Treasury 조회 이전에 거부.

        회귀(#1320): A7 oracle이 검출한 버그 — Signal.stop_price=0,
        order_type='stop' 주문이 RuleEngine→OrderValidatedEvent까지 진행되어
        Treasury 예약 단계에서 거부됐다. 0 stop_price는 무의미한 트리거이므로
        ...
        0, 0.0, -0.0이다. Python에서 0 == 0.0 == -0.0 = True이므로 == 0으로
        모두 잠근다 (선례 #1305/#1318 패턴). -0.0 reason 부호도 audit trail에
        그대로 보존되는지 잠근다.

        OrderRejectedEvent 스키마에 stop_price 필드가 없으므로 reason 문자열에만
        zero stop_price가 등장한다 (audit trail).
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for zero stop_price")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError(
                "_query_treasury_data must not run for zero stop_price"
            )

        async def _spy_unrealized(bot_id, current_price, order_symbol):
            unrealized_calls.append((bot_id, current_price, order_symbol))
            raise AssertionError(
                "_calculate_bot_unrealized_pnl must not run for zero stop_price"
            )
        ...
        order = OrderRequestEvent(
            ...
            stop_price=bad_stop_price,
            exchange="KRX",
            reason="A7 oracle zero-stop_price regression",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        assert "Zero stop_price" in ev.reason
        # f-string은 -0.0의 부호를 보존하므로 reason 안에서 그대로 등장한다.
        assert str(bad_stop_price) in ev.reason
        assert ev.account_id == "domestic"
        assert ev.bot_id == "bot1"
        assert ev.strategy_id == "s1"
        assert ev.symbol == "005930"
    ...
    async def test_rule_engine_rejects_zero_stop_price_stop_limit(
        self, engine, eventbus, monkeypatch
    ):
        """order_type='stop_limit'에서도 zero stop_price는 본 게이트로 거부된다.

        본 게이트는 order_type 무관하게 stop_price==0을 잠그므로, stop_limit 주문에
        zero stop_price가 실려도 동일하게 fail-closed로 거부되어야 한다.
        cross-order_type #1323 회귀(stop_limit zero stop_price terminal_event 누락)를
        함께 잠근다. payload의 price=1000.0은 finite positive이므로 보존된다.
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for zero stop_limit stop_price")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError(
                "_query_treasury_data must not run for zero stop_limit stop_price"
            )
        ...
        order = OrderRequestEvent(
            ...
            stop_price=0,
            exchange="KRX",
            reason="zero stop_price stop_limit regression (#1323)",
        )
        await eventbus.publish(order)
    ...
    @pytest.mark.parametrize("good_stop_price", [0.001, 1, 1000.0])
    async def test_rule_engine_accepts_positive_stop_price_after_zero_gate(
        self, engine, eventbus, good_stop_price
    ):
        """양수 stop_price는 본 게이트(#1320)를 통과해 OrderValidatedEvent 발행.

        본 PR(#1320)은 zero stop_price만 추가로 거부한다. 양수 finite stop_price는
        계속 통과해야 하며, 기존 정상 흐름이 깨지지 않는지 회귀 잠금. 정수/실수,
        매우 작은 양수까지 잠근다. spy를 사용하지 않아 valid 이벤트가 downstream
        evaluate/Treasury까지 도달하는지 함께 검증.
        """
        validated: list[OrderValidatedEvent] = []
        rejected: list[OrderRejectedEvent] = []
        eventbus.subscribe(OrderValidatedEvent, lambda e: validated.append(e))
        ...
        order = OrderRequestEvent(
            ...
            stop_price=good_stop_price,
            exchange="KRX",
            reason="positive stop_price should pass zero-stop_price gate",
        )
        await eventbus.publish(order)
    ...
    async def test_rule_engine_zero_stop_price_fires_before_negative_quantity_gate(
        self, engine, eventbus, monkeypatch
    ):
        """zero stop_price 게이트가 negative quantity 게이트보다 먼저 fire.

        cross-field invariant: stop_price=0 + quantity=-1 + order_type='stop' 주문이
        들어오면 zero-stop_price 게이트(#1320)가 negative-quantity 게이트(#1304)
        보다 먼저 fire되어 reason은 'Zero stop_price ...'가 되어야 한다 (게이트
        선후 순서 잠금). negative-quantity 게이트가 먼저 트리거되면 audit trail의
        근본 원인이 어긋난다.

        payload의 quantity=-1.0은 finite negative이므로 _coerce_finite_quantity가
        그대로 보존하여 audit trail에 실린다 (선례 #1304 negative-quantity 패턴).
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError(
                "evaluate must not run for zero stop_price + negative quantity"
            )

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError(
                "_query_treasury_data must not run for "
                "zero stop_price + negative quantity"
            )
        ...
        order = OrderRequestEvent(
            ...
            stop_price=0,
            exchange="KRX",
            reason="zero stop_price + negative quantity cross-field gate ordering",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        # zero-stop_price 게이트가 우선 fire — negative-quantity 게이트보다 먼저 잡힘
        assert "Zero stop_price" in ev.reason
        assert "Negative quantity" not in ev.reason
        # audit 보존: 음수 finite quantity가 그대로 payload에 실린다.
        assert ev.quantity == -1.0
        assert ev.order_type == "stop"
        assert ev.order_id == str(order.event_id)
    ...
    async def test_rule_engine_rejects_zero_quantity(
        self, engine, eventbus, monkeypatch
    ):
        """quantity=0(int)이면 룰 평가/Treasury 조회 이전에 거부한다.

        회귀(#1305): A7 oracle이 검출한 버그 — Signal.quantity=0, side='buy',
        order_type='limit', price=1000 주문이 RuleEngine→OrderValidatedEvent→
        Treasury까지 진행되어 ``reserve_amount_invalid``(total_reserve=0.0)
        거부가 발생했다. 0 quantity는 no-op 주문이므로 Treasury 예약 호출
        이전에 fail-closed로 거부한다. evaluate/_query_treasury_data/
        _calculate_bot_unrealized_pnl 모두 호출되지 않아야 한다.
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        def _spy_evaluate(context):
            evaluate_calls.append(context)
            raise AssertionError("evaluate must not run for zero quantity")

        async def _spy_query_treasury(bot_id: str = ""):
            treasury_calls.append(bot_id)
            raise AssertionError("_query_treasury_data must not run for zero quantity")

        async def _spy_unrealized(bot_id, current_price, order_symbol):
            unrealized_calls.append((bot_id, current_price, order_symbol))
            raise AssertionError(
                "_calculate_bot_unrealized_pnl must not run for zero quantity"
            )
        ...
        order = OrderRequestEvent(
            ...
            price=1000.0,
            exchange="KRX",
            reason="A7 oracle zero-quantity regression",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        assert "Zero quantity" in ev.reason
        # int 0의 f-string 표현은 ``"0"``
    ...
    @pytest.mark.parametrize(
        ("zero_quantity", "expected_repr"),
        [(0.0, "0.0"), (-0.0, "-0.0")],
    )
    async def test_rule_engine_rejects_zero_float_quantity(
        self, engine, eventbus, monkeypatch, zero_quantity, expected_repr
    ):
        """quantity=0.0/-0.0이면 본 게이트가 fire한다.

        Python에서 ``0 == 0.0 == -0.0``이 True이므로 ``== 0`` 비교가 모두
        잠근다. reason은 f-string으로 ``0.0`` / ``-0.0``를 audit trail에
        보존하여 부호 정보를 잃지 않는다 (#1305 plan v2의 audit 요구사항).
        """
        rejected: list[OrderRejectedEvent] = []
        validated: list[OrderValidatedEvent] = []
        eventbus.subscribe(OrderRejectedEvent, lambda e: rejected.append(e))
        ...
        order = OrderRequestEvent(
            ...
            price=1000.0,
            exchange="KRX",
            reason="A7 oracle zero-float-quantity regression",
        )
        await eventbus.publish(order)
        ...
        ev = rejected[0]
        assert "Zero quantity" in ev.reason
        # f-string으로 부호 보존: ``0.0`` vs ``-0.0``
        assert expected_repr in ev.reason
        assert ev.quantity == 0.0

    @pytest.mark.parametrize("good_quantity", [0.001, 0.5, 1])
    async def test_rule_engine_accepts_positive_quantity_after_zero_gate(
        self, engine, eventbus, good_quantity
    ):
        """양수 quantity는 0-quantity 게이트(#1305)를 통과한다.

        본 PR(#1305)은 ``== 0``만 차단한다. 양수 finite quantity (정수/실수,
        매우 작은 양수까지)는 계속 통과해야 하며, 기존 정상 흐름이 깨지지
        않는지 회귀 잠금. OrderValidatedEvent 발행을 통해 게이트 통과를 잠근다.
        """
        validated: list[OrderValidatedEvent] = []
        ...
        order = OrderRequestEvent(
            ...
            price=1000.0,
            exchange="KRX",
            reason="positive quantity should pass zero-quantity gate",
        )
        await eventbus.publish(order)
