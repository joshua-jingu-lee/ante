...
def _place_noncoercible_partition(data_path: Path) -> Path:
    ...
    발생한다(read는 성공, concat만 실패 = genuine merge-fail). 0바이트 파티션과
    달리 자동복구 대상이 아니라 store_merge 게이트 경로다.
    """
...
class _AnomalyDataGoKrCollector:
    """store 이상(merge 실패 또는 0바이트 복구)을 발생시키는 stub data.go.kr collector.
    ...
    0바이트이면 부재로 간주해 신규 group으로 자동복구하고 `store_recovered` 경고를
    적재한다(#2413). 둘 다 store 경고 버퍼를 통해 표면화된다.
    """
    ...
    async def collect(
        ...
        # merge 실패 시 store.write는 net_delta=0, 0바이트 복구는 net_delta>0.
...
async def test_daily_run_surfaces_store_merge_warning(tmp_path: Path) -> None:
    ...
    **읽기 가능한** 결합-불가 파티션(또는 비-0바이트 손상)에서 발생하므로
    non-coercible fixture로 재현한다(0바이트 파티션은 자동복구 → store_recovered).
    """
...
async def test_daily_run_surfaces_store_recovered_warning(tmp_path: Path) -> None:
    ...
    # 0바이트 파티션을 사전 배치 → 다음 write에서 자동복구 유발.
    part_dir = data_path / "fundamental" / "KRX" / "005930"
    part_dir.mkdir(parents=True)
    (part_dir / "2025-09.parquet").write_bytes(b"")
    ...
    # 0바이트 자동복구는 store_merge를 만들지 않는다(비게이트).
    ...
    assert not (part_dir / "2025-09.corrupted").exists()
