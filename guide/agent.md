...
> 🤖 에이전트가 Ante 전체를 빠르게 파악하려면 루트의 [`llms.txt`](../llms.txt)를, 시스템이 어떻게 동작하는지는 [핵심 개념](concepts.md)을 먼저 보세요.
...
- [핵심 개념](concepts.md) — Ante가 어떻게 동작하는가 (개념·다이어그램·워크플로우)
- [전략 작성](strategy.md) — 전략을 만들고 검증·백테스트하기
