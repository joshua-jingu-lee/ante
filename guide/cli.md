...
| `ante treasury snapshot` | 일별 자산 스냅샷 조회. |
...
| `--from YYYY-MM-DD` | 기간 조회 시작일. |
| `--to YYYY-MM-DD` | 기간 조회 종료일. |
| `--account ACCOUNT_ID` | 계좌 ID로 필터링. |
| `--format json` | JSON 형식 출력. |

`--date`와 `--from`/`--to`는 동시에 사용할 수 없다.
`--from`/`--to`를 생략하면 각각 전체 시작/오늘이 기본값으로 적용된다.
