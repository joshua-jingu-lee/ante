...
| 명령 | 설명 | scope | 토큰 |
|------|------|-------|------|
| `ante account create` | 대화형 계좌 생성. | `account:write` | H·A |
...
| `ante account delete` | 계좌 삭제 (소프트 딜리트). | `account:write` | H·A |
| `ante account credentials` | 인증 정보 조회 (마스킹). | `account:read` | H·A |
| `ante account set-credentials` | 인증 정보 재설정 (대화형 또는 --app-key/--app-secret 옵션). | `account:write` | H·A |
...
| `ante system activate` | 킬 스위치 해제 (거래 재개). | `system:admin` | H·A |
...
| 옵션 | 필수 | 타입 | 기본값 | 설명 |
|------|------|------|--------|------|
| `--app-key` | - | TEXT | — | APP_KEY (비대화형) |
| `--app-secret` | - | TEXT | — | APP_SECRET (비대화형) |
| `--format` | - | text / json | — | 출력 형식 (text 또는 json) |
...
| `--account` | - | TEXT | — | 계좌 ID (미지정 시 대화형 선택) |
...
| 옵션 | 필수 | 타입 | 기본값 | 설명 |
|------|------|------|--------|------|
| `--account-id` | - | TEXT | — | 계좌 ID (미지정 시 'default') |
