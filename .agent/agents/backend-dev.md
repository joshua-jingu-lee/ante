...
1. **이슈, Plan Preflight, Codex Plan Review verdict 확인**: 이슈 본문, 관련 `docs/specs/`, 오케스트레이터가 넘긴 파일 맵 / 작업 분해 / risk flags / verification plan을 먼저 읽는다
2. **영향 범위 파악**: 변경 대상 모듈과 사이드이팩트 발생 가능성을 점검한다
3. **구현**: 스펙과 Codex Plan Review verdict에 따라 코드를 작성한다
4. **테스트 작성**: `tests/unit/`에 테스트를 추가하고 `pytest` 실행으로 검증한다
5. **린트 확인**: `ruff check`와 `ruff format --check`로 코드 스타일을 검증한다

## Codex Plan Review 게이트

- 오케스트레이터가 Codex Plan Review verdict를 전달하기 전까지 구현을 시작하지 않는다.
...
- 구현 중 새 risk flag가 추가로 드러나면, 바로 코드를 더 밀지 말고 오케스트레이터에 Codex Plan Review 재호출을 요청한다.
...
- **Codex Plan Review 준수**: verdict가 허용한 범위 밖으로 확장하지 않는다
