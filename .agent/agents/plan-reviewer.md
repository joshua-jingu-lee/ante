...
6. **검증·롤백 공백**: Verification이 추상 문장이 아니라 실행 가능한 check인가. 중간 상태가 안전한가.
   ...
   - **본문 사본 ↔ SSOT 정합**: 이슈 본문 `### Verification`의 `집행 규칙 블록`이 현재 SSOT의 판정 관용구와 같은가. 개발 에이전트가 실행하는 것은 SSOT가 아니라 본문 사본이라, SSOT 갱신 전에 쓰인 낡은 사본(예: 매처 플래그 없는 `grep -c`)은 옛 시맨틱으로 실행된다.
