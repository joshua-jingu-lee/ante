...
6. **검증·롤백 공백**: Verification이 추상 문장이 아니라 실행 가능한 check인가. 중간 상태가 안전한가.
   회귀 락(grep·diff 기반 기계적 검사)이 있으면 아래 건전성 축까지 본다. 판정 기준 SSOT는 `.agent/commands/plan-preflight.md`의 「회귀 락 설계 규칙」이다.
   - **vacuous pass**: 아무것도 고치지 않은 상태에서도 통과하는 락이 아닌가. 전역 grep·토큰 존재 검사는 형제 이슈가 같은 파일에 넣은 텍스트로 충족될 수 있다.
   - **양방향 자기모순**: 계획이 지시한 구현을 락이 거부하지 않는가. baseline before/after 대조표가 실측으로 실려 있는가.
   - **앵커 건전성**: awk 범위의 시작 앵커가 파일 안에서 유일하고 종료 앵커가 존재하는가. 범위 재개방·EOF 폭주와 라인 번호 드리프트가 막혀 있는가.
   - **census 완전성**: 락이 덮을 표면 집합이 census로 닫혔는가. 서로 다른 탐색 양식으로 훑었고 히트 전건이 수정 또는 명시 제외로 판정됐는가.
   - **본문 사본 ↔ SSOT 정합**: 계획이 이슈 본문 `### Verification`에 싣는 락 판정 관용구가 현재 SSOT의 정본 형태와 같은가. 개발 에이전트가 실행하는 것은 SSOT가 아니라 본문 사본이라, SSOT 갱신 전에 쓰인 낡은 사본(예: 매처 플래그 없는 `grep -c`)은 옛 시맨틱으로 실행된다.
