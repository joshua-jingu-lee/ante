...
- effort는 `high`·`xhigh`·`max` 중 하나로 명시한다(레벨 순서는 `high` < `xhigh` < `max`).
...
- 호출 직전 `git status --porcelain` 출력이 비어 있음을 확인한다. 비어 있지 않으면 미커밋 변경을 커밋하거나 제거해 비운 뒤 호출하며, 확인 결과가 증적의 `worktree:` 필드 값이다.
