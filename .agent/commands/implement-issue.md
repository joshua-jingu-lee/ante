...
- 호출 직전 리뷰 대상 브랜치가 체크아웃된 워크트리의 `git status --porcelain` 출력이 비어 있음을 확인한다. 비어 있지 않으면 개발 서브에이전트에게 커밋을 위임해 비운 뒤 `{head}`와 range를 재산출해 호출한다.
- 재호출은 같은 이슈의 직전 호출과 같은 레벨 이상으로 한다. 직전 호출의 effort를 증적에서 확인할 수 없으면 하한은 `high`다.
