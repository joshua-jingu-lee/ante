...
- **값을 하드코딩하지 말고 패턴을 잠근다.** 버전·라인 번호·SHA는 리뷰가 도는 동안 드리프트하는 표면이다. `# v1.14.1` 대신 `grep -E '@[0-9a-f]{40} # v[0-9]+\.[0-9]+\.[0-9]+'`처럼 형태를 잠근다. **ERE 메타문자(`{n}`·`+`·`|`)를 쓰면 `-E`를 붙인다.** bare `grep`은 basic regex라 이 문자들이 리터럴로 해석돼 매치가 0건이 되고, 긍정형 락은 false FAIL, 부정형(`! … grep -q`)은 조용한 vacuous PASS가 된다.
