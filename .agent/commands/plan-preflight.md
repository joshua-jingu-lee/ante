...
- **보조 — 섹션 불변은 hunk 파싱이 아니라 바이트 동일성으로 잠근다.** 두 스냅샷을 직접 비교하면 라인 번호가 밀려도 안전하다. **종료 앵커는 다음 헤딩으로 두고 위의 배타형과 결합하되, 판정은 아래 집행 규칙의 정본 형태로 감싼다.** `diff <(…) <(…)`를 bare로 한 줄 두면 판정 명령이 되어 금지되고, **양쪽 앵커가 다 없으면 두 스냅샷이 모두 비어 rc=0으로 통과한다**(실측: 존재하지 않는 앵커로 양쪽을 뽑으면 `diff`가 rc=0). 그래서 **두 스냅샷 각각이 비어 있지 않음을 먼저 확인**한다.
...
- **판정은 정본 형태 하나로만 쓴다 — 긍정형과 부정형이 같은 모양이고 `test` 비교 연산자만 다르다.**
  ...
  **다른 판정 형태를 쓰지 않는다.** idiom을 여럿 두면 각각이 `set -euo pipefail` 아래에서 서로 다른 엣지 케이스를 갖고, 계획 작성자가 그 단서를 전부 지키지 못한다. 판정 줄은 예외 없이 `test <비교> || { echo 'FAIL: …'; exit 1; }` 꼴이며, 생산자가 `grep -c`든 `wc -l`이든 판정 줄의 모양은 바뀌지 않는다. 아래 넷은 **이 한 형태가 동시에 닫는 함정**이고, 쓰라는 지시가 아니라 **왜 이 형태인지의 근거**다.
