...
- **판정은 정본 형태 하나로만 쓴다 — 긍정형과 부정형이 같은 모양이고 `test` 비교 연산자만 다르다.** **계수는 `grep -cE`가 기본이다** — 정본 형태를 그대로 복사해도 ERE 문법이 리터럴로 죽지 않고, BRE 의미가 필요할 때만 그때 `-E`를 뺀다.
...
- **락이 만드는 로그·임시 파일은 워크트리 안의 gitignore된 경로(`docs/temp/`·`/logs/`)에 둔다.** 공유 `/tmp`는 동시에 도는 다른 작업과 충돌해 결과를 오염시킨다. **gitignore되지 않은 경로**에 두면 락이 자기가 측정하는 상태를 오염시킨다 — 로그가 untracked 파일로 남아 clean 워크트리 요구를 깨고, `git add -A`에 쓸려 들어가면 정확 파일 수 락까지 FAIL시킨다.
