...
- **보조 — 섹션 불변은 hunk 파싱이 아니라 바이트 동일성으로 잠근다.** 두 스냅샷을 직접 비교하면 라인 번호가 밀려도 안전하다. **종료 앵커는 다음 헤딩으로 두고 위의 배타형과 결합한다**: `diff <(git show "$B:$F" | awk '/^## 9\./{f=1} /^## 10\./{f=0} f') <(awk '/^## 9\./{f=1} /^## 10\./{f=0} f' "$F")`. **종료 앵커를 `0`으로 두는 `awk '/^## 9\./,0'` 형태는 §9가 파일의 마지막 섹션일 때만 쓴다** — awk의 `0`은 어떤 행에도 매치하지 않아 범위가 EOF까지 폭주한다(실측: `printf '## 9. A\nbody9\n## 10. B\nbody10\n' | awk '/^## 9\./,0'`은 `## 10. B`와 `body10`까지 출력한다). §9 뒤에 다른 섹션이 있으면 이 락이 그 섹션들까지 함께 동결하므로, 계획이 §10 변경을 지시했는데 「§9 불변」 락이 그 구현을 거부하는 양방향 자기모순이 된다. `,0`을 유지하려면 「§9가 마지막 섹션」이라는 전제를 계획에 적고 그 전제 자체를 락으로 확인한다.
...
- **락 스크립트에는 `#!/usr/bin/env bash` shebang을 넣거나 `bash <파일>`로 실행한다.** 프로세스 치환(`diff <(…) <(…)`)·배열 같은 bash 전용 문법이 쓰이므로 `sh <파일>`로 돌리면 문법 오류가 나고, 느슨한 하니스에서는 그 오류가 하니스 실패가 아니라 락 실패로 읽힌다. 스크립트 첫머리에 `set -euo pipefail`도 함께 둔다 — base 커밋 변수가 unset이면 range diff가 조용히 워크트리 diff로 퇴화한다.
  - **경고 — `set -o pipefail`은 파이프 뒤의 `grep -q`와 결합하면 불건전하다.** `grep -q`는 첫 매치에서 즉시 종료하고, 파이프 앞의 생산자(`awk`·`git show`·`git diff`·`seq`)는 아직 쓸 게 남아 있으면 SIGPIPE로 죽으며, `pipefail`이 그 **141**을 파이프라인 종료 상태로 전파한다. 실측: `seq 1 2000000 | grep -q '^1$'` → rc 141이라 **긍정형 락이 false FAIL**하고, 금지 토큰을 담은 같은 크기 입력의 부정형은 rc 0이라 **vacuous PASS**한다. 더 나쁜 것은 이 동작이 **생산자 출력이 파이프 버퍼를 넘느냐에 달렸다**는 점이다 — 같은 락이 대상 파일이 커진 뒤 조용히 반대로 뒤집힌다(같은 명령을 `seq 1 5`로 줄이면 rc 0으로 통과한다).
  - **회피형 셋 중 하나를 쓴다.** ① **`grep -c`로 세어 `test`로 비교한다**(`N=$(생산자 | grep -c '패턴'); test "$N" -ge 1`) — `grep -c`는 입력을 끝까지 읽으므로 SIGPIPE가 나지 않는다. ② **`grep -q`를 파이프가 아니라 파일 인자나 here-string에 쓴다**(`grep -q '패턴' "$F"` / `grep -q '패턴' <<<"$X"`) — 파이프가 없으니 전파할 141도 없다. ③ 먼저 변수에 담는 `X=$(awk …)` → `printf '%s\n' "$X" | grep -q …` 형태는 **추출 구간이 파이프 버퍼보다 작을 때만** 안전하다. 절 하나를 뽑는 락은 이 범위에 들지만, 파일 전체나 대용량 diff를 담으면 ①·②로 바꾼다.
- **패턴은 항상 홑따옴표로 감싼다.** 큰따옴표 안의 백틱과 `$`는 셸이 먼저 해석해 패턴을 바꿔 버린다.
- **부정 조건은 `if cmd | grep -q '금지패턴'; then echo 'FAIL: …'; exit 1; fi` 형태로 쓴다. `! cmd` 형태는 쓰지 않는다.** `set -e`는 **`!`로 반전된 명령의 실패를 무시**한다 — bash 매뉴얼이 명시한 예외(「the command's return value is being inverted with `!`」)다. 따라서 `! cmd | grep -q '금지토큰'`은 금지 토큰이 **실제로 존재해도** 스크립트를 중단시키지 못하고, 다음 줄로 그대로 진행해 exit 0으로 끝난다. 실측: `set -euo pipefail` 아래 `! echo "FORBIDDEN" | grep -q FORBIDDEN`은 후속 `echo`를 실행하고 스크립트 종료 상태가 0이다. **부정형 락이 잡아야 할 것을 잡고도 비집행이 되는** 것이라, 이 규칙 집합이 막으려는 vacuous PASS 그 자체다. 스크립트 안의 마지막 한 줄만 예외적으로 종료 상태에 반영되므로, 부정형이 여러 개면 마지막 하나를 뺀 전부가 죽는다. 위 `if` 형태는 조건절 안이라 `set -e`가 적용되지 않는 자리를 쓰면서도 명시적 `exit 1`로 집행되고, `echo 'FAIL: …'`이 어느 락이 깨졌는지도 남긴다.
- **`grep -qv`는 부정 판정에 쓰지 않는다.** 위와 별개 사유다 — 「매치하지 않는 행이 하나라도 있으면 참」이라 의도한 「전 행 부재」를 판정하지 못한다.
