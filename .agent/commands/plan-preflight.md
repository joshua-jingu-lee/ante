...
- **보조 — `git diff --exit-code -- <경로>`는 조건부 가드다.** 이 비교는 **워크트리 ↔ 인덱스**라 regenerate 순서와 스테이징 시점에 **모두** 의존한다. regenerate 없이 실행하면 양쪽이 같아 통과하고, regenerate한 뒤라도 그 결과를 스테이징하면 다시 통과한다(실측: `git add <경로>` 직후 같은 명령이 rc=0). 이 가드를 락으로 쓰려면 그 두 조건이 실제로 성립하는지를 계획이 직접 확인하고, 판정은 집행 규칙의 정본 형태로 감싼다. 저장소 기존 처방의 정합은 **#2472**가 다룬다.
...
- **보조 — 섹션 불변은 hunk 파싱이 아니라 두 스냅샷 직접 비교로 잠근다.** 라인 번호가 밀려도 안전하다. **종료 앵커는 다음 헤딩으로 두고 위의 배타형과 결합하되, 판정은 아래 집행 규칙의 정본 형태로 감싼다.** `diff <(…) <(…)`를 bare로 한 줄 두면 판정 명령이 되어 금지되고, **양쪽 앵커가 다 없으면 두 스냅샷이 모두 비어 rc=0으로 통과한다**(실측: 존재하지 않는 앵커로 양쪽을 뽑으면 `diff`가 rc=0). 그래서 **두 스냅샷 각각이 비어 있지 않음을 먼저 확인**한다. 이 비교가 잠그는 것은 **바이트 동일성의 유계 근사**다 — `A=$(…)`가 후행 개행을 전부 버리고 `printf '%s\n'`이 하나만 되붙이므로 **섹션 끝 빈 줄이 늘거나 줄어든 변경은 동일로 판정된다**(실측: 후행 빈 줄만 다른 두 입력이 diff 0건인데 `cmp`는 rc=1). 이 근사로 충분한지는 계획이 판단해 적는다.

  ```bash
  A=$(git show "$B:$F" | awk '/^## 9\./{f=1} /^## 10\./{f=0} f')
  C=$(awk '/^## 9\./{f=1} /^## 10\./{f=0} f' "$F")
  test -n "$A" || { echo "FAIL: base 스냅샷이 비었다 — 앵커 확인"; exit 1; }
  test -n "$C" || { echo "FAIL: 현재 스냅샷이 비었다 — 앵커 확인"; exit 1; }
  S=$(diff <(printf '%s\n' "$A") <(printf '%s\n' "$C") || true)
  N=$(printf '%s\n' "$S" | grep -c '^[<>]' || true)
  test "$N" -eq 0 || { echo "FAIL: §9가 변했다"; exit 1; }
  ```
...
- **판정은 정본 형태 하나로만 쓴다 — 긍정형과 부정형이 같은 모양이고 `test` 비교 연산자만 다르다.**
  ...
  **다른 판정 형태를 쓰지 않는다.** idiom을 여럿 두면 각각이 `set -euo pipefail` 아래에서 서로 다른 엣지 케이스를 갖고, 계획 작성자가 그 단서를 전부 지키지 못한다. 판정 줄은 예외 없이 `test <비교> || { echo 'FAIL: …'; exit 1; }` 꼴이며, 생산자가 무엇을 세든 판정 줄의 모양은 바뀌지 않는다. 아래 넷은 **이 한 형태가 동시에 닫는 함정**이고, 쓰라는 지시가 아니라 **왜 이 형태인지의 근거**다.
  ...
  - **`grep -c`는 0건일 때 rc=1이라 그대로 대입하면 대입 줄에서 스크립트가 죽는다.** 대입의 종료 상태가 곧 `grep`의 rc이기 때문이다(실측: `set -euo pipefail` 아래 `N=$(printf 'a\nb\n' | grep -c 'ZZZ')`는 다음 줄을 출력하지 못하고 exit 1). **`|| true`가 이것을 중화**해 0건을 판정 실패가 아니라 `N=0`이라는 측정값으로 넘긴다.
  - **`grep -q`는 `pipefail`과 결합하면 입력 크기에 따라 판정이 뒤집힌다.** `grep -q`는 첫 매치에서 즉시 종료하고, 파이프 앞 생산자(`awk`·`git show`·`git diff`·`seq`)는 쓸 게 남아 있으면 SIGPIPE로 죽으며 `pipefail`이 그 **141**을 파이프라인 종료 상태로 전파한다. 실측: 금지 토큰을 담은 2,000,000줄 입력에 `if seq 1 2000000 | grep -q '^1$'; then echo FAIL; exit 1; fi`는 **`PASS`를 찍고 exit 0**(vacuous PASS)인데, 같은 명령을 `seq 1 5`로 줄이면 정상 FAIL한다 — **대상이 커진 뒤 같은 락이 조용히 반대로 뒤집힌다.** 정본 형태의 `grep -c`는 **입력을 끝까지 읽으므로** SIGPIPE가 나지 않는다(실측: 같은 2,000,000줄 입력을 `S=$(seq 1 2000000)`로 담아 `printf '%s\n' "$S" | grep -c`에 넣어도 긍정형은 `N=1`로 통과, 부정형은 141이 아니라 exit 1로 집행).
  ...
  - **`grep -qv`는 부재 판정에 쓰지 않는다.** 「매치하지 않는 행이 하나라도 있으면 참」이라 「전 행 부재」를 판정하지 못한다. 부재는 위 부정형(`-eq 0`)으로만 판정한다. 반대로 매치하지 않는 행을 **세는** `grep -vc`는 정본 형태의 생산자 안에서 쓴다 — 그것은 판정이 아니라 계수다.
...
- **락이 만드는 로그·임시 파일은 워크트리 안의 gitignore된 경로(`docs/temp/`·`/logs/`)에 둔다.** 공유 `/tmp`는 동시에 도는 다른 작업과 충돌해 결과를 오염시킨다. 추적되는 경로에 두면 락이 자기가 측정하는 상태를 오염시킨다 — untracked 로그가 clean 워크트리 요구를 깨고, `git add -A`에 쓸려 들어가면 정확 파일 수 락까지 FAIL시킨다.
- **계획 작성자는 이 집행 주의 블록을 이슈 본문에 실어 보낸다.** 개발 에이전트는 실행 시점에 이 커맨드 문서를 읽지 않고 이슈 본문의 verification을 읽는다. 본문에 없으면 집행 규칙은 소비되지 않는다.

##### 검증 규칙

- **계획 확정 전에 baseline에서 전 락을 실제로 실행하고 before/after 대조표를 계획에 싣는다.** 「지금은 실패, 고치면 통과」가 실측으로 확인되지 않은 락은 판별력이 없다. 계획 확정 시점에는 구현이 없으므로 「after」는 **스크래치 사본에 임시 패치를 적용해 측정한 뒤 그 사본을 폐기**해서 만든다. 작업 브랜치에 구현을 미리 넣어 두는 방식은 쓰지 않는다.
