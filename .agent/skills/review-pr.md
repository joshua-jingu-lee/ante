...
> PR에 대해 구조화된 체크리스트 기반 코드 리뷰를 수행하고, 판정 결과를 PR 코멘트로 게시한다.
> 리뷰어는 **판정만** 수행한다. 수정과 머지는 리뷰어의 역할이 아니다.

## 트리거

- `/review-pr #{PR번호}` — 사용자가 수동 실행
- `/implement-issue` 12단계에서 PR 생성 후 자동 실행
...
**리뷰어는 여기서 끝난다.** 수정과 머지는 오케스트레이터가 조율한다.
