...
- 이슈 close는 GitHub 기본 auto-close를 우선 사용하고, `post-merge` automation은 체크박스와 에픽 상태를 동기화한다. **연결 이슈 본문**의 체크박스 갱신은 **`완료 조건` 헤딩 아래 절에만** 적용되며, 이슈 폼의 `위험 신호`처럼 완료 조건이 아닌 체크박스는 건드리지 않는다. 에픽 본문의 `## 하위 작업` 체크리스트는 그와 별개 경로로, 하위 이슈 번호를 지목해 그 줄만 갱신한다. 에픽 소속 판정은 §3.4가 정한 `에픽: #N` 마커를 읽는다.
