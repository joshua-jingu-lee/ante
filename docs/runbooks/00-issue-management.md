...
- 이슈 close는 GitHub 기본 auto-close를 우선 사용하고, `post-merge` automation은 체크박스와 에픽 상태를 동기화한다. **연결 이슈 본문**의 체크박스 갱신은 **`완료 조건` 헤딩 아래 절에만** 적용되며(절 경계는 `#` 헤딩으로 판정하고, 헤딩 제목은 이슈 폼이 만드는 `완료 조건`으로 시작하고, 뒤에 설명을 붙이려면 낱말이 아니라 `:`·`(`·`—` 같은 문장부호로 이어야 한다 — `완료 조건 매핑`·`완료 조건 2차`·`완료 조건_초안`은 절을 켜지 않는다), 이슈 폼의 `위험 신호`처럼 완료 조건이 아닌 체크박스는 건드리지 않는다. 에픽 본문 갱신은 이와 별개 경로다.
