...
| 마커 | 뜻 | 소비자 |
|------|-----|--------|
| `에픽: #N` | 소속 선언 | `post-merge.yml`이 이 문자열로 부모 에픽을 찾아 체크리스트를 갱신하고 auto-close를 판정한다(`Epic: #N`도 같은 값으로 읽는다). 이 표가 그 마커의 SSOT이며, 마커가 없으면 에픽 동기화는 발화하지 않는다 |
...
- 이슈 close는 GitHub 기본 auto-close를 우선 사용하고, `post-merge` automation은 체크박스와 에픽 상태를 동기화한다. 체크박스 갱신은 **`완료 조건` 헤딩 아래 절에만** 적용되며, 이슈 폼의 `위험 신호`처럼 완료 조건이 아닌 체크박스는 건드리지 않는다. 에픽 소속 판정은 §3.4가 정한 `에픽: #N` 마커를 읽는다.
- **본문 전체를 치환하던 시기(#2509 이전)에 close된 이슈의 체크박스 상태는 `완료 조건` 절 밖이면 근거로 쓰지 않는다**(`위험 신호`·`유저스토리`·계획 `Tasks` 등). 실제 해당 여부와 무관하게 모두 「해당함」으로 기록됐다. 원본은 GitHub 편집 이력에 남아 있으며, 닫힌 이슈 본문은 소급 정정하지 않는다.
