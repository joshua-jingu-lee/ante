...
- 이슈 close는 GitHub 기본 auto-close를 우선 사용하고, `post-merge` automation은 체크박스와 에픽 상태를 동기화한다. **연결 이슈 본문**의 체크박스 갱신은 **`완료 조건` 헤딩 아래 절에만** 적용되며, 이슈 폼의 `위험 신호`처럼 완료 조건이 아닌 체크박스는 건드리지 않는다. 에픽 본문 갱신은 이와 별개 경로다.
- **본문 전체를 치환하던 시기(#2509 이전)에 close된 이슈의 체크박스 상태는 `완료 조건` 절 밖이면 근거로 쓰지 않는다**(`위험 신호`·`유저스토리`·계획 `Tasks` 등). 그중 **컬럼 0의 `- ` 불릿 항목은 코드 펜스 안이든 산문 중이든 가리지 않고** 실제 해당 여부와 무관하게 「해당함」으로 기록됐다(들여쓴 항목과 `*`·`+` 불릿만 그때도 갱신되지 않았다). 원본은 GitHub 편집 이력에 남아 있으며, 닫힌 이슈 본문은 소급 정정하지 않는다.
