...
- **본문 전체를 치환하던 시기(#2509 이전)에 close된 이슈의 체크박스 상태는 `완료 조건` 절 밖이면 근거로 쓰지 않는다**(`위험 신호`·`유저스토리`·계획 `Tasks`, 그리고 `Closes #에픽번호` PR로 닫힌 에픽의 `## 하위 작업` 등). 실제 해당 여부와 무관하게 모두 「해당함」으로 기록됐다. 원본은 GitHub 편집 이력에 남아 있으며, 닫힌 이슈 본문은 소급 정정하지 않는다.
