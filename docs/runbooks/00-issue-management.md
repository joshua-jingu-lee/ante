...
- **본문 전체를 치환하던 시기(#2509 이전)에 close된 이슈의 체크박스 상태는 `완료 조건` 절 밖이면 근거로 쓰지 않는다**(`위험 신호`·`유저스토리`·계획 `Tasks` 등). 그 시기 자동화는 절도 코드 펜스도 가리지 않았고, 어떤 항목을 켰는지는 불릿 형태와 들여쓰기에 따라 갈려 개별 판별이 어렵다. 원본은 GitHub 편집 이력에 남아 있으며, 닫힌 이슈 본문은 소급 정정하지 않는다.
