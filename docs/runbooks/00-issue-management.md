...
| `process` | (운영) 개발 프로세스를 정의하거나 집행하는 표면만 바꾸는 이슈: 에이전트 운영 문서(`.agent/`·`.claude/`·`AGENTS.md`, `CLAUDE.md`는 그 심볼릭 링크)·런북(`docs/runbooks/`)·프로세스 ADR·이슈 폼(`.github/ISSUE_TEMPLATE/`)·merge-gate/post-merge 워크플로·기여 정책(`CONTRIBUTING.md`). 제품 계약(`docs/specs/`)·제품 코드와 제품 테스트·제품 CI/릴리스 워크플로(`ci.yml`·`publish.yml`·`semantic-release.yml`)는 아니다. 운영 장치 변경이 강제하는 생성 산출물·생성기 갱신은 이 판정을 바꾸지 않는다. `autopilot.md` 「기본 제외 대상」이 이름을 든다 | — |
