...
| 마커 | 뜻 | 소비자 |
|------|-----|--------|
| `에픽: #N` | 소속 선언 | `post-merge.yml`이 이 문자열로 부모 에픽을 찾아 체크리스트를 갱신하고, 에픽 본문에 미체크 `- [ ]`가 하나도 남지 않으면 에픽을 close한다(`Epic: #N`도 같은 값으로 읽는다). **그 close 판정은 절을 가리지 않아 `위험 신호` 같은 하위 작업 아닌 체크박스도 센다 — 판정 정의역 재설계는 #2523이 소유한다.** 이 표가 그 마커의 SSOT이며, 마커가 없으면 에픽 동기화는 발화하지 않는다. **에픽 본문에 그 하위 이슈 번호를 담은 `- [ ] … #N` 줄이 함께 있어야** 체크리스트가 갱신된다(그 줄을 `## 하위 작업`에 두는 것은 이 절의 규약이고, 갱신 자체는 절을 가리지 않는다) |
