...
| 마커 | 뜻 | 소비자 |
|------|-----|--------|
| `에픽: #N` | 소속 선언 | `post-merge.yml`이 이 문자열로 부모 에픽을 찾아 체크리스트를 갱신하고 auto-close를 판정한다(`Epic: #N`도 같은 값으로 읽는다). 이 표가 그 마커의 SSOT이며, 마커가 없으면 에픽 동기화는 발화하지 않는다. **에픽 본문 `## 하위 작업`에 그 하위 이슈 번호를 담은 `- [ ] … #N` 줄이 함께 있어야** 체크리스트가 갱신된다 |
