...
| `process` | (운영) 개발 프로세스를 정의하거나 집행하는 표면만 바꾸는 이슈: 에이전트 운영 문서·런북·프로세스 ADR·이슈 폼·merge-gate/post-merge 자동화·기여 정책. 제품 계약·제품 코드와 제품 테스트·제품 CI/릴리스는 아니며, 운영 장치 변경이 강제하는 생성 산출물 갱신은 판정을 바꾸지 않는다. `autopilot.md` 「기본 제외 대상」이 이름을 들어 **자동 큐에서만 제외**하며 수동 `/implement-issue`는 막지 않는다. 판정 예시·보정 사례는 #2521 | — |
