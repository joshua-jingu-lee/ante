...
- autopilot 큐 판정에서 타입 라벨과 area 라벨이 모두 없는 이슈에 오케스트레이터가 이 라벨을 부착한다. 판정 절차·라벨 집합의 SSOT는 [.agent/commands/autopilot.md](../../.agent/commands/autopilot.md) 「큐 선별 규칙」이며, 이 경로로 붙은 라벨은 타입 또는 area 라벨을 부착해 분류한 뒤 제거한다.
