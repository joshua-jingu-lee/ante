...
1. 제목이 [00-issue-management.md](00-issue-management.md) §2의 `[{type}]` 규약을 따르고 그 `{type}`이 **위 표 `feat`~`chore` 7행의 이슈 타입**(`feat`·`fix`·`refactor`·`perf`·`docs`·`test`·`chore`) 중 하나이면, 위 표에서 그 타입 행의 prefix를 쓴다. 이 목록은 표의 사본이므로 §2에 타입을 더하거나 빼면 이 줄을 같은 PR에서 함께 고친다.
...
두 갈래가 산출하는 prefix는 갈래 1의 7종과 `chore/`뿐이고, 현재 §3.1 브랜치 리뷰 트리거 대상 글롭이 그 8종을 모두 포함한다(실측). 따라서 이 규칙으로 만든 브랜치는 Gate A가 정상 트리거되고, 라벨이 비어 있다는 이유로 사전 리뷰가 건너뛰어지지 않는다. **§3.1의 대상 브랜치 목록에서 이 8종 중 하나라도 빠지면 이 문장이 거짓이 되고 그 prefix의 Gate A가 조용히 건너뛰어진다** — §3.1 목록을 바꿀 때는 이 절을 같은 PR에서 함께 고친다.

#### `enhancement` 라벨의 `feat/`·`perf/` 판정

위 표 `대응 라벨` 열에서 `enhancement`는 `feat` 행과 `perf` 행 **두 곳**에 걸려 있어, 그 라벨을 가진 이슈는 표만으로 prefix가 결정되지 않는다. 이 이슈들은 타입 라벨을 가지므로 앞 절의 정의역 밖이지만, 표도 결정하지 못하므로 규칙을 여기에 둔다.

- 제목이 §2의 `[perf]` 규약을 따르면 `perf/`를 쓴다.
- 그 외 `enhancement` 이슈는 `feat/`를 쓴다.

커밋 `<type>`은 앞 절과 같이 결정된 prefix와 같은 토큰을 쓴다(`perf/` → `perf(...)`, `feat/` → `feat(...)`). 두 타입의 §2 커밋 버전 범프가 다르므로(`feat`=minor, `perf`=patch) 이 판정은 릴리스 버전에 영향을 준다. 폼 템플릿이 제목을 프리필하므로 폼 경로로 등록된 이슈는 이 규칙으로 결정적으로 갈린다.
