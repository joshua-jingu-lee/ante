...
- `/code-review = PASS`
  - 브랜치 push 후 PR 생성 가능
- `/code-review = FAIL`
  - Claude가 같은 워크트리에서 수정한 뒤 §3.1의 호출 형태 그대로 재검토 — 첫 호출과 같은 effort 레벨 토큰을 다시 붙인다
...
- 추가 AI 감사가 필요하면 사람/오케스트레이터가 같은 브랜치 리뷰를 수동으로 다시 호출하고(호출에는 effort 레벨 토큰을 붙인다), 그 결과를 PR 코멘트에 남긴다. 자동 PR 승인 워커는 더 이상 동작하지 않는다.
