...
> lane은 두 가지다: **maintainer/collaborator 내부 lane**과 **외부 fork lane**. Gate 0/A/C는 내부 lane 전용이며, Gate B의 공개 CI는 두 lane 공통이다.
...
외부 fork PR에서는 same-repo guard 때문에 `merge-gate`가 정상적으로 skip된다. 이때는 시크릿 없는 GitHub-hosted read-only CI만 실행한다.
...
D-020 필수 운영값 세 가지는 운영 GitHub 저장소 설정에서 유지·확인한다. 두 lane 공통 보호 권장값과 내부 same-repo 운영 권장값은 운영자가 GitHub UI 또는 API로 적용한다. workflow가 저장소 설정을 직접 변경하지 않는다.
...
- 알려진 실패 모드:
  ...
  - **`[done-criteria:heading-not-found] #N`**: 본문 어딘가에 불릿 미체크 항목(`-`·`*`·`+`, 선행 공백 3칸까지)이 있는데 `완료 조건` 헤딩을 찾지 못했다. 그 항목이 `위험 신호`처럼 완료 조건이 아닌 것뿐이면 갱신할 것이 애초에 없었다는 뜻이라 조치가 필요 없다.
  - **`[done-criteria:unterminated-fence] #N`**: 코드 펜스가 열린 채 본문이 끝나 `완료 조건` 절이나 그 항목이 그 안에 가려졌을 수 있다. 절을 정상 갱신했더라도 **그 절 안에서** 예시 블록이 닫히지 않았으면 발화하고, 절이 끝난 뒤 열린 펜스에는 발화하지 않는다.
...
- 복구 순서:
  ...
  3. **`[done-criteria:*]` 경고면** 먼저 위 실패 모드 설명으로 **조치 대상인지 가른다** — 미체크 항목이 애초에 완료 조건이 아니었거나 항목이 아닌 예시가 잡힌 경우는 갱신할 것이 없었다는 뜻이라 그대로 둔다. 조치 대상이면 본문의 펜스·헤딩·항목 형태를 고치지 않는 한 재실행해도 같은 결과라, 아래 멱등성의 「이미 `[x]`면 무해」와 달리 반복만으로는 상태가 진전되지 않는다. 기록을 맞출 필요가 있다고 판단하면 본문을 고친 뒤 2번을 돌리되, 아래 멱등성 항이 정한 조건에서 `정리 완료` 코멘트가 중복 게시될 수 있다. 판단이 서지 않으면 닫힌 이슈 본문은 그대로 두는 것이 기본이다([00-issue-management.md](00-issue-management.md) §6).
