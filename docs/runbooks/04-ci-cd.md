...
- 알려진 실패 모드:
  ...
  - **`[done-criteria:*] #N`**: 연결 이슈 #N의 `완료 조건` 체크리스트가 갱신되지 않았거나 일부만 갱신됐다. 태그가 원인을 가리킨다 — `heading-not-found`(절을 찾지 못함) · `unterminated-fence`(코드 펜스가 절이나 그 항목을 가림) · `item-not-matched`(절 안에 인식하지 못한 형태의 항목). 각 태그의 정확한 발화 조건은 `post-merge.yml`의 `markDoneCriteriaChecked`가 정본이고 이 줄은 그 요약이다.
- 복구 순서:
  ...
  3. **`[done-criteria:*]` 경고면** 그 이슈의 체크리스트는 갱신되지 않았다. **닫힌 이슈 본문은 소급 정정하지 않으므로**([00-issue-management.md](00-issue-management.md) §6) 그 기록은 그대로 두고, 같은 형태가 반복되면 본문 작성 관행을 고친다.
