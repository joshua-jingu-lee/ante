...
- 알려진 실패 모드:
  ...
  - **`[done-criteria:item-not-matched] #N`**: `완료 조건` 절은 찾았으나 인식하지 못하는 형태의 미체크 항목이 있어 갱신에서 빠졌다(들여쓰기 4칸 이상·탭·번호 목록·불릿 뒤 공백 2칸 이상).
- 복구 순서:
  ...
  3. **`[done-criteria:*]` 경고면** 그 이슈의 체크리스트가 갱신되지 않았거나 일부만 갱신된 것이다. 본문의 펜스·헤딩·항목 형태를 고치지 않으면 재실행해도 같은 결과라, 아래 멱등성의 「이미 `[x]`면 무해」와 달리 반복만으로는 상태가 진전되지 않는다. 기록을 맞출 필요가 있다고 판단하면 본문을 고친 뒤 2번을 돌리되, 이미 정리된 이슈라 `정리 완료` 코멘트가 중복 게시된다(아래 멱등성 참조). 판단이 서지 않으면 닫힌 이슈 본문은 그대로 두는 것이 기본이다([00-issue-management.md](00-issue-management.md) §6).
