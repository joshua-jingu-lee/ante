...
> 본 섹션은 스테이징 환경 기준의 요약이다. 로그 시스템의 공식 스펙은 [docs/specs/core/core.md](../../specs/core/core.md)의 "Logging 인터페이스" 절이 SSOT이며, 필드 스키마·fingerprint 규칙·핸들러 구성·회전 정책은 거기서 관리한다. 본 섹션은 그 요약 인용이다.
...
| 스펙 | 보강 내용 |
|---|---|
| [docs/specs/core/core.md](../../specs/core/core.md) | "Logging 인터페이스" 절이 신설되어 로그 3종(system/event/audit) 관계, JSONL 필드 스키마, fingerprint 규칙, 핸들러·회전 정책을 SSOT로 관리한다. 본 문서 §6은 이 스펙의 요약 인용. |
...
별도 모듈 카테고리를 신설하지 않고 `docs/specs/core/core.md`의 "Logging 인터페이스" 절로 관리한다. 근거: 로깅은 Database와 동일하게 "모든 모듈이 공유하는 기반 인프라"이므로 Core 모듈의 자산으로 분류하는 것이 자연스럽다.
