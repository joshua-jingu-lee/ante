...
> 본 섹션은 스테이징 환경 기준의 요약이다. 로그 시스템의 공식 스펙은 [docs/specs/core/logging/](../../specs/core/logging/)가 SSOT이며, 필드 스키마·fingerprint 규칙·핸들러 구성·회전 정책은 거기서 관리한다. 본 섹션은 그 요약 인용이다.
...
| 스펙 | 보강 내용 |
|---|---|
| [docs/specs/core/logging/](../../specs/core/logging/) | 신설 분할 스펙. 로그 3종 관계, JSONL 스키마, fingerprint 규칙, 핸들러·회전 정책, 컨텍스트 필드 주입 패턴, 구현 위치를 주제별 문서로 관리한다. 본 문서 §6은 이 스펙의 요약 인용. |
...
별도 모듈 카테고리를 신설하지 않고 `docs/specs/core/logging/` 하위로 주제별 분할 관리한다. 근거: 로깅은 Database와 동일하게 "모든 모듈이 공유하는 기반 인프라"이므로 Core 모듈의 자산으로 분류하는 것이 자연스럽다. 분할 형식은 [docs/specs/account/](../../specs/account/)의 다중 파일 패턴을 따른다 (`README.md` 인덱스 + `<module>.md` 호환 문서 + `0N-<topic>.md` 주제별 문서).
