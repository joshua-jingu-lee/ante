...
> 본 섹션은 스테이징 환경 기준의 요약이다. 로그 시스템의 공식 스펙은 별도 카테고리 [docs/specs/logging/](../../specs/logging/) (신설 예정)에 둔다. 필드 스키마, fingerprint 규칙, 핸들러 구성, 회전 정책은 해당 스펙이 SSOT이며 본 섹션은 그 요약 인용이다.
...
| 스펙 | 보강 내용 |
|---|---|
| **`docs/specs/logging/`** (신설) | 신규 모듈 스펙 카테고리. 로그 3종(system/event/audit) 관계 정리, JSONL 필드 스키마, fingerprint 규칙, 핸들러·회전 정책. [docs/specs/README.md](../../specs/README.md) 인덱스 갱신. 본 문서 §6은 이 스펙의 요약 인용이 된다. |
...
### 10.1 `docs/specs/logging/` 구조 (확정)

```
docs/specs/logging/
├── README.md                    # 인덱스
├── 01-overview.md               # 로그 3종(system/event/audit) 관계
├── 02-design-decisions.md       # JSONL 채택, 이중 핸들러, ANTE_LOG_JSONL 게이트
├── 03-json-schema.md            # 필수/선택 필드, Exception 필드, fingerprint 규칙
├── 04-handlers-and-rotation.md  # stdout 평문 / file JSONL, TimedRotatingFileHandler
├── 05-context-fields.md         # extra={} 주입 패턴 (account_id, bot_id 등)
├── 06-cross-module-notes.md     # event_log (eventbus) / audit_log (audit)와 역할 구분
└── logging.md                   # 호환 문서 (개요 + 앵커)
```
