...
- **Bot 스펙**: `ante bot create/start/stop/remove`와 `signal-key --rotate`는 IPC를 통해 서버의 BotManager를 호출하는 런타임 커맨드다. 실행 중 조회(`list/info/status/positions/signal-key`)도 서버 live 상태가 필요하면 IPC를 우선 사용한다.
