...
| POST | `/api/bots` | 봇 생성. Body: bot_id, strategy_id?, strategy_name?, name?, account_id?, interval_seconds?, budget?. **strategy 식별자 (#1436):** `strategy_id` 또는 `strategy_name` 중 하나는 필수(Pydantic `model_validator`로 강제). 둘 다 전달 시 `strategy_id`를 우선 사용한다. **schema (#1436):** Pydantic `extra='forbid'` + OpenAPI `additionalProperties: false` — 정의되지 않은 필드(legacy `bot_type` 포함)는 422로 거부된다. `budget`은 finite positive number (Infinity/NaN은 422로 거부, #1435). 계좌 정지 상태이면 409 Conflict. **budget 처리 (#1335):** budget 포함 시 봇 생성 후 Treasury 배정을 시도한다. 배정이 `TreasuryError`(insufficient funds, treasury not configured for account 등)로 실패하면 422를 반환하고 신규 봇은 `delete_bot(handle_positions="keep")`로 best-effort 롤백된다. 롤백 자체가 실패하면 500과 함께 `budget allocation failed: ... rollback also failed; bot {bot_id} may be in partial state. Check bot status and treasury manually.` detail이 노출된다. 비-`TreasuryError`는 422로 매핑하지 않고 propagate한다 (bot create + budget 배정은 atomic transaction이 아닌 best-effort 롤백 정책). |
