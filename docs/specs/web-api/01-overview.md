...
- **HTTP 인증 표면**: human dashboard는 세션 쿠키, agent/API client는 Bearer token을 사용한다.
...
- 인증·권한 모델 결정 → Web API는 HTTP transport 추출과 endpoint 적용만 담당, principal/scope 판정은 [member.md](../member/member.md)가 SSOT
