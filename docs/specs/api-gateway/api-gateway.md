...
- **`_on_order_approved`는 `trading_mode != LIVE`이면 broker 라우팅을 deterministic하게 skip한다**(account_service에서 trading_mode 조회). EventBus 동일 priority(=50) 구독순서에 의존하는 'VirtualExecutor consume' 방식은 **금지**(APIGateway가 먼저 등록되면 consume marker 전에 `place_order` 호출되는 race로 `kis-domestic+virtual` 회귀 락이 깨짐). 현 gateway가 virtual approval도 `place_order`를 호출하는 불일치를 이 trading_mode skip으로 해소한다(#2398 요구).
...
- **virtual 시장가 가격 안전(#2398)**: `VirtualExecutor`의 시장가 체결가는 `APIGateway.get_current_price()`(broker 의존)로 조회하며 `OrderApprovedEvent.price`는 시장가 매수에서 `None`이다. broker 장애 중 가격 조회 실패가 `event.price or 0.0` 폴백으로 **0원 체결**되지 않도록, **가격 조회 실패 시 terminal reject/fail(0원 체결 금지)**로 명시한다(broker-independent 가격 resolver 가용 또는 명시적 실패 처리). virtual broker_ready 면제는 이 가격 안전 요구와 짝을 이룬다.
