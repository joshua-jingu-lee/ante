...
| `modify_order(order_id, quantity, price, reason) → None` | 주문 정정 (백테스트에서는 noop). 라이브는 **v1=price-only 지원(#2391)** — `open` 주문 가격 정정 시 Gateway가 broker 위임 후 `modified` 통보. 수량 변경·예산증가 buy·부분체결/터미널 등은 fail-closed 거부(고급=#2393) |
