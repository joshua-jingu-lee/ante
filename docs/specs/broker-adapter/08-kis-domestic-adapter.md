...
- **교차 구간(`from < cutoff <= to`) bounded known-limitation**: cutoff를 가로지르는 창은 split query 없이 start-date 기준 before 단일 쿼리로 처리한다(완전성은 known-limitation). 호출자는 둘이다 — 자동 경로인 `FillReconcileScheduler`는 항상 ≤7일 창이라 inner 고정이지만, **사용자 표면 `ante broker order-history`(#2412)는 임의 `--from`을 받으므로 before 분기와 교차 구간이 실제로 도달 가능**하다. 이 명령은 교차 구간을 거부하지 않고 `--help`와 CLI 스펙에 한계로 노출만 한다(정책 변경 없음). 교차 구간 fail-closed 거부를 어댑터 소유로 배치하는 것은 `BrokerAdapter` 계약 변경이라 별건이다.
