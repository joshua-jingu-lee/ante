...
[broker-adapter.md](broker-adapter.md)는 기존 링크와 섹션 앵커 호환을 위한 인덱스이며 계약 본문이 아니다.
새 계약, 결정, 미결 사항은 [broker-adapter.md](broker-adapter.md)에 추가하지 않고 해당 하위 문서에 반영한다.

| 문서 | 내용 |
|---|---|
| [broker-adapter.md](broker-adapter.md) | 호환용 인덱스 및 기존 섹션 앵커 (계약 본문 아님) |
