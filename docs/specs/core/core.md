...
| Live DataCollector write·경로 생성 | **OHLCV `{1m,5m,15m,1h}`만** (`1d`는 write-ownership상 DataFeed 소유라 제외, `tick`은 별도 data_type라 제외) | enforcement 구현 (#1614 완료, `src/ante/data/collector.py:_is_valid_ingress`) | ingress enforcement는 `_is_valid_ingress`가 `is_valid_timeframe`(#1613), `timeframe != "1d"` write-ownership guard, `is_krx_symbol`(#1613) 검사로 구현 (#1614 완료, #1613 derive). 본 행은 "Collector write vocabulary는 OHLCV `{1m,5m,15m,1h}`로 한정(축 D상 `1d`·축 C상 `tick` 제외)" 경계만 명시 |
