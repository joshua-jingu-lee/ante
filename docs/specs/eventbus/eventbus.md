...
| `OrderModifyRejectedEvent` | RuleEngine / APIGateway | Bot | `account_id`, `order_id`, `bot_id`, `strategy_id`, `symbol`, `side`, `quantity`, `price?`, `reason`. 정정 거부 이벤트 (룰 거부 사유 또는 v1 fail-closed: `modify_invalid_args`/`modify_qty_change_unsupported`/`modify_budget_increase_unsupported`/`modify_partial_or_terminal_unsupported`/`modify_not_owner`/`modify_unsupported_order_type`/`modify_orgno_unavailable`/`modify_failed`) |
