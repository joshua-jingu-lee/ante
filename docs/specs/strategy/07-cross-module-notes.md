...
- **CLI 스펙 작성 시**: 명령 시그니처와 실행 분류의 SSOT는 [cli/03-commands.md](../cli/03-commands.md)다. Strategy 모듈은 `validate/submit/list/info/performance`의 도메인 동작만 설명한다. `ante backtest run <파일경로>` 커맨드는 등록 없이 전략 파일을 직접 로드하여 백테스트 실행 — Agent의 반복 개발 루프 지원
